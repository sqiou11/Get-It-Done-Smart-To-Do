 ...
 * **Added in v.4.3.8**
 *
 * Synchronously generates a Node.js module with an SQL tree, based on a configuration object, which has the following format:
 *
 * ```js
 * {
 *    // REQUIRED
 *    
 *    "dir" // {string}: relative or absolute directory where SQL files are located;
 *          // see API for method enumSql, parameter `dir`.
 *
 *    // OPTIONAL
 *    
 *    "recursive" // {boolean}: search for sql files recursively;
 *                // see API for method enumSql, option `recursive`.
 *
 *    "ignoreErrors" // {boolean}: ignore common errors;
 *                   // see API for method enumSql, option `ignoreErrors`.
 *
 *    "output" // {string}: relative or absolute destination file path
 *     
 *    "module": {
 *        "path" // {string}: relative path to a module exporting a function which takes a file
 *               // path and returns a proper value (typically, a new QueryFile object);
 *               // by default, it uses `./loadSql`.
 *
 *        "name" // {string}: local variable name for the SQL-loading module;
 *               // by default, it uses `load`.
 *    }
 * }
 * ```
 *
 * This method is normally used on a watch that triggers when files change in your SQL directory,
 * or it can be used manually.
 *
 * @param {object|string} [config]
 * Configuration parameter for generating the code.
 *
 * - When it is a non-null object, it is assumed to be a configuration object.
 * - When it is a text string - it is the relative path to a JSON file that contains the configuration object.
 * - When `config` isn't specified, the method will try to locate the default `sql.json` file in the same folder as
 ...
 * @example
 *
 * // generate SQL module automatically,
 * // based sql.json in the module's root:
 *
...
function buildSqlModule(config) {
    ...
    // root = folder with the file executed by Node.js
    ...
    if (config) {
        ...
        var defConfig = path.join(root, 'sql.json');
        // istanbul ignore else;
        if (!fs.existsSync(defConfig)) {
            throw new Error("Default SQL configuration file not found: " + defConfig);
        }
        // cannot test this automatically, because it requires that file 'sql.json'
